...
class CallIO(Protocol):
    ...
    (AGI today, ARI/Stasis next) supplies a concrete implementation. Keeping
    the surface this small is what lets the same flow logic run under either
    driver — and be exercised in tests with an in-memory fake.

    `media` arguments are driver-scoped identifiers (an Asterisk stream name
    like ``"voicemail/busy"`` under AGI, a ``sound:``/``recording:`` URI under
    ARI). Core never constructs them; it only replays what the caller passed
    in, so the naming scheme stays the driver's concern.
    """
